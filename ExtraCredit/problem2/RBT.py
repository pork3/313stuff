...
class RedBlackTree:
	...
	def sub_size1(self, node):
		size = 0
		if node == self.nil:
			return 1
			print(size)
		l = self.sub_size1(node.left)
		r = self.sub_size1(node.right)
	...
	def k_small(self, node , k):
		r = node.subtreesize
		if k == r:
			return node
		if k < r:
			return self.k_small(node.left, k)
		if k > r:
			return self.k_small(node.right , k)

	def small_lab(self , node , k):
		if i <= node.left.subtreesize:
			self.small_lab(node.left, k)
		if i > node.left.size+1:
			self.small_lab(node.left , i - node.left.subtreesize)
		else:
			return node
